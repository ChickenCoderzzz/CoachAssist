...
export default function PlaybookPage() {
  ...
  const reloadBoards = () => {
    fetch(`/teams/${teamId}/drawboards?scope=playbook`, { headers: authHeaders() })
      .then((r) => r.json())
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        window.alert(`Create failed: ${t}`);
        return;
      }
      const data = await res.json();
      setNewTitle("");
      setShowCreate(false);
      navigate(`/team/${teamId}/playbook/${data.drawboard.id}`);
    } finally {
      setCreating(false);
    }
  };
        ...
        <div className="team-grid">
          {boards.map((b) => (
            <div
              key={b.id}
              className="game-card"
              onClick={() => navigate(`/team/${teamId}/playbook/${b.id}`)}
            >
              {canEdit && (
                <button
